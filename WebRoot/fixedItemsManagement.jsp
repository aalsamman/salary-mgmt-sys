...
<script src="js/lightbox-form3.js" type="text/javascript"></script>
  ...
  <body>
   <header class="clearfix">
	...
	<div class="component">
		<%List FixedSalariesList=(List)request.getAttribute("FixedSalariesList"); 
		%>
		<table>
			<thead>
				<tr>
  					...
  					<th>Employee Name</th>
  					<% 
  						int index = ((FixedSalary)FixedSalariesList.get(0)).getEmployeeId();
  						for (int i = 0; i<FixedSalariesList.size(); i++)
  						{
  							FixedSalary fixedItem =(FixedSalary)FixedSalariesList.get(i);
  							
  							if(fixedItem.getEmployeeId()==index){		
    				%>
    				<th><a href="javascript:void(0)" onClick="openboxx('Edit Value', 1,<%=request.getAttribute("departmentID") %>,<%=fixedItem.getSalaryItemId() %>)"><%=fixedItem.getSalaryItemName()%></a></th>
    				<%}}%>
				</tr>
			</thead>
				<tbody>
				<%
					...
					for (int i=0; i<FixedSalariesList.size();i++){
					FixedSalary fixedItem =(FixedSalary)FixedSalariesList.get(i); 
					if(prevId!=fixedItem.getEmployeeId()){
					prevId = fixedItem.getEmployeeId();
					
				%>
				<tr>
					<th><%=fixedItem.getEmployeeId() %></th>
					<td><%=fixedItem.getEmployeeName() %></td>
					<%
						for (int j=0; j<FixedSalariesList.size();j++){
						FixedSalary temp =(FixedSalary)FixedSalariesList.get(j); 
							if(temp.getEmployeeId()==fixedItem.getEmployeeId()){
								if(temp.getValue()!=0){
					%>
